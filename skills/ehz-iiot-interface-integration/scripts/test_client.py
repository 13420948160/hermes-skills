...
API 类型：
- /ESBREST/faas/ 或 /ESBREST/edge/ 前缀：errorCode/errorMsg/return 格式
...
class TestRunner:
    ...
    def _is_faas_api(self, path: str) -> bool:
        ...
        return path.startswith(FAAS_PATH_PREFIX)

    def _is_edge_api(self, path: str) -> bool:
        """判断是否为 edge 类型接口（直接 JSON body，不需要 apikey 包装）"""
        return path.startswith(EDGE_PATH_PREFIX)
    ...
    def _do_request(self, ep: ApiEndpoint, payload: dict) -> tuple[dict, int]:
        ...
        try:
            if self._is_faas_api(ep.path):
                wrapped = {"apikey": "", "request": payload}
                resp = requests.post(url, headers=headers, json=wrapped, timeout=10)
            elif self._is_edge_api(ep.path):
                # edge 类型：直接 JSON body，不需要 apikey 包装
            ...
            return {"errorCode": 408, "errorMsg": "请求超时", "success": False, "message": "请求超时"}, int((time.time() - start) * 1000)
        except ReadTimeout:
            return {"errorCode": 408, "errorMsg": "读取响应超时", "success": False, "message": "读取响应超时"}, int((time.time() - start) * 1000)
        except ReqConnectionError as e:
            return {"errorCode": 503, "errorMsg": f"无法连接到服务器: {e}", "success": False, "message": str(e)}, int((time.time() - start) * 1000)
        except requests.exceptions.HTTPError as e:
            status = e.response.status_code if e.response is not None else 500
            return {"errorCode": status, "errorMsg": f"HTTP错误: {status}", "success": False, "message": f"HTTP {status}"}, int((time.time() - start) * 1000)
        except Exception as e:
            return {"errorCode": 500, "errorMsg": str(e), "success": False, "message": str(e)}, int((time.time() - start) * 1000)
    ...
    def _validate_response(self, resp: dict, api_type: str) -> tuple[bool, str]:
        ...
        if api_type == "iiot":
            ...
            return True, ""
        elif api_type in ("faas", "edge"):
            if "errorCode" not in resp:
                return False, f"{api_type} 类型响应缺少必要字段 errorCode，实际: {list(resp.keys())}"
            return True, ""
        return False, f"未知的 API 类型: {api_type}"
    ...
    def _extract_params(self, ep: ApiEndpoint) -> dict:
        ...
        if ep.request_example and isinstance(ep.request_example, dict):
            # faas 类型的 request 是包装在 request 字段里的
            if ep.request_example.get("request"):
                inner = ep.request_example["request"]
            ...
            if p.name not in payload:
                ...
                if default:
                    ...
                    if p.name == "page":
                        ...
                        payload[p.name] = "3710e1d4-7a76-40d0-84a5-81224d3fe892"
                    elif p.name == "addr":
                        payload[p.name] = "insertProductionPlan"
                    elif p.name == "appName":
                        payload[p.name] = "业务云函数管理"
                    elif p.name == "id" and "debugAppInterface" in ep.path:
                        payload[p.name] = "a80c558e-dff3-4976-aad4-37d0384abd11"
                    elif p.name == "inParam":
                        payload[p.name] = [{"code": "order_id", "type": "string", "defaultVal": ""}]
                    elif p.name == "callWay":
                        payload[p.name] = "debug"
    ...
    def _run_single_test(self, ep: ApiEndpoint) -> TestResult:
        ...
        logger.info("[%s] 开始测试: %s (%s) payload=%s", ep.method, ep.name, api_type, payload)
        ...
        for attempt in range(1, MAX_RETRIES + 1):
            ...
            try:
                ...
                # 验证响应格式
                is_valid, val_err = self._validate_response(resp, api_type)

                if is_valid:
                    # 进一步检查：iiot 类型 success 字段，faas 类型 errorCode == 0
                    if api_type == "iiot":
                        ...
                        if not passed:
                            is_valid = False
                            val_err = f"{api_type} 接口调用失败: errorCode={resp.get('errorCode')}, errorMsg={resp.get('errorMsg')}"
                ...
                if is_valid:
                    ...
                    logger.warning("[%s] %s 失败 (第 %d/%d 次): %s", api_type.upper(), ep.name, attempt, MAX_RETRIES, val_err)
    ...
    def print_report(self, report: TestReport, output_file: str = None):
        ...
        sep = "=" * 80
        lines.append(sep)
        lines.append("极联平台接口管理集成 API 兼容性测试报告")
        lines.append(sep)
    ...
    def verify_endpoints_loaded(self) -> list[ApiEndpoint]:
        ...
        for ep in endpoints:
            api_type = "faas" if self._is_faas_api(ep.path) else "iiot"
            logger.info("  [%d] %s %s (%s)", ep.index, ep.method, ep.path, api_type)
        return endpoints
...
def main():
    parser = argparse.ArgumentParser(description="极联平台接口管理集成 API 兼容性测试")
    ...
    if args.list:
        print("\n可用测试用例:")
        for ep in endpoints:
            api_type = "faas" if ep.path.startswith(FAAS_PATH_PREFIX) else "iiot"
            print(f"  [{ep.index}] {ep.name} ({api_type}) - {ep.method} {ep.path}")
        return
