...
class ApiDocsParser(BaseMarkdownParser):
    ...
    def _parse_section(self, text: str) -> ApiEndpoint | None:
        ...
        req_ex = self._parse_json_block(text, "**入参示例**:")
        ...
        # 根据路径前缀判断 API 类型
        if path.startswith("/ESBREST/faas/"):
            api_type = "faas"
        else:
            api_type = "iiot"
        notes_match = re.search(
    ...
    def _parse_json_block(self, text: str, marker: str) -> dict | None:
        ...
        # marker 为字面字符串（非正则），re.escape 将特殊字符转义后拼接正则后缀
        escaped = re.escape(marker)
        pattern = escaped + r"\s*```(?:json)?\s*([\s\S]+?)\s*```"
