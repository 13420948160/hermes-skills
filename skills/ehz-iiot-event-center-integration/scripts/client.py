#!/usr/bin/env python3
"""极联平台事件中心集成动态 CLI 客户端（自动从 api_docs.md 生成）"""
import logging
import os
import sys
...
CMD_ALIASES = {
    # 事件中心申明相关
    "event-define-list": "事件中心申明表-分页列表查询",
    "event-define-export-all": "事件中心申明表-分页列表查询-导出专用",
    # 事件中心动作相关
    "event-action-list": "事件中心动作表-分页列表查询",
    "send-to-event-message": "事件中心动作表-发布事件消息到指定队列类型",
}


class EhzEventCenterClient(DynamicApiClient):
    """极联平台事件中心集成动态客户端，支持两种 API 类型"""
    ...
    # 过滤掉空值参数，避免后台将空字符串转为 "Default value must not be null" 错误
    # actionType 有效值: kafka / rabbitmq / set / service / script（不是 http）
...
def main():
    ...
    client = EhzEventCenterClient(
        parser_cls=ApiDocsParser,
        api_docs_path=api_docs,
        skill_name="ehz-iiot-event-center-integration",
