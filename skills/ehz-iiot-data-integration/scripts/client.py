#!/usr/bin/env python3
"""极联平台数据集成动态 CLI 客户端（自动从 api_docs.md 生成）"""
import logging
import os
import sys
...
# CLI 别名映射（CLI 命令 -> 端点名）
CMD_ALIASES = {
    "data-storage-list": "数据存储管理-列表查询",
    "list-data-storage": "数据存储管理-列表查询",
    "get-table-structure": "数据存储管理-获取表结构数据",
    "get-table-struct": "数据存储管理-获取表结构数据",
    "get-tags-tree": "系统-标签表-获取标签树",
    "get-refer-table-data": "获取关联表数据",
    "table-field-list": "数据存储管理-获取表字段信息",
    "get-table-field-list": "数据存储管理-获取表字段信息",
    "get-table-relationship-list": "数据存储管理-查询所有表关系列表",
    "get-relationship-list": "数据存储管理-查询所有表关系列表",
    "get-table-relationship-detail": "获取关系详情接口",
    "get-relationship-detail": "获取关系详情接口",
}


class EhzDataIntegrationClient(DynamicApiClient):
    """极联平台数据集成动态客户端，支持两种 API 类型"""

    # /ESBREST/iiot/ 前缀使用 success/message/code/result 格式
    IIOT_PATH_PREFIX = "/ESBREST/iiot/"
    # /ESBREST/faas/ 前缀使用 errorCode/errorMsg/return 格式
...
def main():
    ...
    client = EhzDataIntegrationClient(
        parser_cls=ApiDocsParser,
        api_docs_path=api_docs,
        skill_name="ehz-iiot-data-integration",
