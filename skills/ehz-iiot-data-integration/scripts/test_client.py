...
- /ESBREST/faas/ 前缀：errorCode/errorMsg/return 格式
"""
import argparse
import json
import logging
...
class TestRunner:
    ...
    def _validate_response(self, resp: dict, api_type: str) -> tuple[bool, str]:
        ...
        if api_type == "iiot":
            ...
            if "errorCode" not in resp:
                return False, f"faas 类型响应缺少必要字段 errorCode，实际: {list(resp.keys())}"
            return True, ""
        ...
        for p in ep.parameters:
            if p.name not in payload:
                ...
                if default:
                    ...
                    # 必填但无默认值，使用已知的测试有效值
                    if p.name == "table_id":
                        payload[p.name] = "HZ_TESTaa"
                    elif p.name == "id":
                        payload[p.name] = 19  # 从 getTableRelationshipList 获取的有效 ID
    ...
    def print_report(self, report: TestReport, output_file: str = None):
        ...
        sep = "=" * 80
        lines.append(sep)
        lines.append("极联平台数据集成 API 兼容性测试报告")
        lines.append(sep)
    ...
    def verify_endpoints_loaded(self) -> list[ApiEndpoint]:
        ...
        for ep in endpoints:
            logger.info("  [%d] %s %s (%s)", ep.index, ep.method, ep.path, ep.name)
        return endpoints
...
def main():
    parser = argparse.ArgumentParser(description="极联平台数据集成 API 兼容性测试")
